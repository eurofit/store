...
type PaginatedBrandProductsProps = {
  slug: string;
...
export default function PaginatedBrandProducts({
  slug,
  initialProducts = [],
  totalProducts,
  userId,
  pathname: href = `/brands/${slug}`,
}: PaginatedBrandProductsProps) {
  ...
  return (
    <div className="mb-10">
      ...
      <nav className="sr-only" aria-label="Pagination navigation for brands">
        {page > 1 && (
          <a href={`${href}?page=${page - 1}&limit=20`} rel="prev">
            Previous
          </a>
        )}
        {products.length < totalProducts && (
          <a href={`${href}?page=${page + 1}&limit=20`} rel="next">
            Next
          </a>
        )}
      </nav>
