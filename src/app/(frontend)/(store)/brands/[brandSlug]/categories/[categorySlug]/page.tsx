...
// TODO: fix this page to show products of brands/slug/cegories
...
export async function generateMetadata({
  ...
  const [brand, category, totalBrandProductLines] = await Promise.all([
    getBrand({ slug: brandSlug }),
    getCategory({ slug: brandSlug }),
    getTotalProductLinesByBrand(brandSlug),
  ]);

  const categoryPluralized = pluralize(category.title, 2);

  return {
    title: `Buy ${brand.title} ${categoryPluralized}  in Kenya`,
